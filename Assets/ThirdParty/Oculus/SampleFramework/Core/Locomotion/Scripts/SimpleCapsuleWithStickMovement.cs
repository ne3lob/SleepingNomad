...
public class SimpleCapsuleWithStickMovement : MonoBehaviour
{
    ...
    void LookUpDown()
    {
        if (OVRInput.IsControllerConnected(OVRInput.Controller.RTouch))
        {
            ...
            // Adjust the vertical input to control the camera's pitch
            Vector3 ortEuler = CameraRig.centerEyeAnchor.localRotation.eulerAngles;
            float newPitch = Mathf.Clamp(ortEuler.x - verticalInput * Speed, 0f, 90f);
            ortEuler.x = newPitch;
            CameraRig.centerEyeAnchor.localRotation = Quaternion.Euler(ortEuler);
        }
    }
